...
ELSE IF 当前会话已执行过 /wf-search:
  → 复用已有搜索结果和上下文
  → 提示用户："已复用之前 /wf-search 的结果，是否重新搜索？"
  → 如果用户选择重新搜索，执行下方搜索流程

ELSE:
  → 自动提取关键词
  → 搜索 {paths.wiki}/{wiki.structure.current}/
  → 计算匹配度
...
按照 references/spec.md 模板生成（使用 config.language 指定的语言）：

```
{paths.changes}/active/{slug}/spec.md
```
...
按照 references/tasks.md 模板生成（使用 config.language 指定的语言）：

```
{paths.changes}/active/{slug}/tasks.md
```
